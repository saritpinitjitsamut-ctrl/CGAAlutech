...
function formatProductionNumber(dateValue, sequence) {
    const orderDate = dateValue ? new Date(dateValue) : new Date();
...
async function generateProductionNumber(orderDateValue, existingValue = null) {
    ...
    const prefix = `${String(orderDate.getDate()).padStart(2, '0')}/`;

    if (!dbSupabase) return `${prefix}1`;
    ...
    try {
        const { data, error } = await dbSupabase
            ...
            .ilike('production_number', `${prefix}%`);
...
async function generateProductionNumberForUpload(orderDateValue, existingValue = null) {
    ...
    if (productionNumberUploadCache[prefix]) {
        const seq = productionNumberUploadCache[prefix]++;
        return `${prefix}${seq}`;
    }
    ...
    const currentSeq = parseInt(result.split('/').pop() || '1', 10);
    // Initialize cache to next seq
    productionNumberUploadCache[prefix] = currentSeq + 1;
...
function getNextProductionNumberForUpload(orderDateValue) {
    ...
    if (isNaN(orderDate.getTime())) return null;
    const prefix = `${String(orderDate.getDate()).padStart(2, '0')}/`;
    if (!productionNumberUploadCache[prefix]) productionNumberUploadCache[prefix] = 1;
    const seq = productionNumberUploadCache[prefix]++;
    return `${prefix}${seq}`;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const orderDateInputForProduction = document.getElementById('orderDate');

    async function fillProductionNumber() {
        if (!productionNumberInput || productionNumberInput.value.trim()) return;
        const generated = await generateProductionNumber(orderDateInputForProduction?.value || null);
        if (generated) productionNumberInput.value = generated;
    }

    if (productionNumberInput && orderDateInputForProduction) {
        orderDateInputForProduction.addEventListener('change', fillProductionNumber);
        productionNumberInput.addEventListener('focus', fillProductionNumber);
        fillProductionNumber();
    }
    ...
    submitBtn.addEventListener('click', async (e) => {
        ...
        try {
            ...
            const productionNumber = await generateProductionNumber(
                document.getElementById('orderDate')?.value || null,
                normalizeString(document.getElementById('productionNumber')?.value)
            );
    ...
    async function processExcelData(rawData) {
        ...
        for (let i = 0; i < cleanData.length; i++) {
            ...
            productionNumber = await generateProductionNumberForUpload(date, productionNumber);
    ...
    if (bulkUploadBtn) {
        bulkUploadBtn.addEventListener('click', async () => {
            ...
            try {
                // 1. ตรวจสอบข้อมูลที่มีอยู่แล้วตามหมายเลขพัสดุเท่านั้น (ไม่ใช้ order_number)
                ...
                parsedDataToUpload.forEach(item => {
                    ...
                    if (t) {
                        // Exact tracking match in DB => SKIP
                    ...
                    // For any other case (including no tracking or no match), always INSERT as a new row.
                    // This allows "Order Number can repeat" as per user request.
                    itemsToInsert.push({...item, tracking_number: t});
                });

                const skipCount = skippedDbDup;
