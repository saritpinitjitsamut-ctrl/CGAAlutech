...
function renderOrderCard(o) {
    ...
    if (o.payment_time) {
        try {
            ...
            timeDetails.push(`<div style="font-size:0.75rem;color:#60a5fa;margin-top:2px;">💳 ชำระ: ${pd.toLocaleDateString('th-TH')} ${pd.toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })}</div>`);
        } catch (e) {
    ...
    <div class="order-card ${cardCls}" id="card_${o.id}">
                    ...
                    try {
                        ...
                        if (o.payment_time) {
                            try {
                                const pd = new Date(o.payment_time);
                                if (!isNaN(pd.getTime())) {
                                    out += `<br><span style="font-size:0.8rem;color:var(--muted);">💳 ${pd.toLocaleDateString('th-TH')} ${pd.toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })}</span>`;
                                }
                            } catch (e) {}
                        }
                        return out;
        ...
        <div style="margin-top:10px;padding:8px 12px;background:rgba(129,140,248,0.12);border:1px solid rgba(129,140,248,0.3);border-radius:8px;font-size:1.05rem;font-weight:700;color:#a5b4fc;display:flex;align-items:center;justify-content:space-between;gap:8px;">
            <span>🧾 เลขที่การผลิต:</span>
            <span style="font-size:1.55rem;color:#fbcfe8;font-family:monospace;letter-spacing:1px;text-shadow:0 0 10px rgba(129,140,248,0.5);">${esc(productionNumberLabel)}</span>
        </div>
