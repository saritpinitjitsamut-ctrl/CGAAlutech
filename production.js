...
function openComponentsModal(order, comps) {
    return new Promise((resolve) => {
        ...
        const hasZeroStock = comps.some(c => (parseInt(c.have) || 0) <= 0);
        const warningHtml = hasZeroStock
            ? `<div style="margin-bottom:12px;padding:12px;border-radius:10px;background:rgba(239,68,68,0.12);color:#b91c1c;border:1px solid rgba(239,68,68,0.2);font-size:0.9rem;">
                    ❌ พบรายการที่มีสต็อก 0 ชิ้น ระบบไม่อนุญาตให้ยืนยันและตัดสต็อกได้ กรุณาเพิ่มสต็อกก่อน</div>`
            : '';
        ...
        comps.forEach(c => {
            ...
            const short = `<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.04);gap:12px;">
                        ...
                        <div style="display:flex;align-items:center;gap:8px;font-size:0.82rem;white-space:nowrap;">
                            ...
                            <span style="background:${c.have < c.need ? 'rgba(239,68,68,0.12)' : 'rgba(16,185,129,0.12)'}; 
                                         color:${c.have < c.need ? '#f87171' : '#34d399'}; 
                                         border:1px solid ${c.have < c.need ? 'rgba(239,68,68,0.22)' : 'rgba(16,185,129,0.22)'}; 
        ...
        confirmBtn.disabled = hasZeroStock;
        confirmBtn.style.cursor = hasZeroStock ? 'not-allowed' : '';
        confirmBtn.title = hasZeroStock ? 'ไม่สามารถยืนยันได้เมื่อวัสดุบางรายการมีสต็อก 0' : '';
...
function renderOrderCard(o) {
    ...
    if (status === STATUS_PENDING) {
        ...
        actionsHtml = `
            ...
            ${canFinish ? `<button class="btn btn-done btn-sm" onclick="updateStatus('${o.id}','${STATUS_DONE}',this)">✅ ผลิตสำเร็จ</button>` : ''}`;
    } else {
