...
function renderLogs() {
    ...
    // Render Issues (OUT)
    if (issues.length === 0) {
        issueLogBody.innerHTML = `<tr><td colspan="5" style="text-align:center; color:var(--muted); padding:2rem;">ไม่มีประวัติการจ่ายออก</td></tr>`;
    } else {
        issueLogBody.innerHTML = issues.map(l => {
            const dateStr  = new Date(l.created_at).toLocaleString('th-TH', { hour12: false });
            const opName   = l.dispatched_by || l.operator || '—';
            const reason   = l.reason || '—';
            return `
            <tr>
                <td style="color:#64748b; font-size:0.8rem;">${dateStr}</td>
