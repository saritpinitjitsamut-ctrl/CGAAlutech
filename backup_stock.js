...
async function openAddSkuModal() {
    ...
    document.getElementById('skuReason').value = '';
...
function switchModalTab(tab) {
    ...
    const tabSearch = document.getElementById('modalTabSearch');
    const tabGen = document.getElementById('modalTabGenerate');
    const panelSearch = document.getElementById('addSkuSearchPanel');
    const panelGen = document.getElementById('addSkuGeneratePanel');

    if (tabSearch && tabGen) {
        tabSearch.classList.toggle('active', tab === 'search');
        tabGen.classList.toggle('active', tab === 'generate');
    }

    if (panelSearch && panelGen) {
        panelSearch.style.display = tab === 'search' ? '' : 'none';
        panelGen.style.display = tab === 'generate' ? '' : 'none';
    }

    const selectEl = document.getElementById('skuSelect');

    if (tab === 'search') {
        if (selectEl) selectEl.setAttribute('required', 'true');
        onSkuSelectChange();
    } else {
...
function initializeSkuGeneratorOptions() {
    const ref = window.SKU_REFERENCE;
    if (!ref) return;

    const setOptions = (id, items, valueKey, labelBuilder, preferredValue) => {
        const select = document.getElementById(id);
        if (!select) return;
        select.innerHTML = items.map(item => `<option value="${esc(item[valueKey])}">${esc(labelBuilder(item))}</option>`).join('');
        if (preferredValue && Array.from(select.options).some(option => option.value === preferredValue)) {
            select.value = preferredValue;
        }
    };

    setOptions('genType', ref.productTypes, 'code', item => `${item.name} (${item.code})`, 'SLD');
    setOptions('genFrameColor', ref.frameColors, 'code', item => `${item.name} (${item.code})`, 'B');
    setOptions('genGlassColor', ref.glassColors, 'code', item => `${item.name} (${item.code})`, 'C');
    setOptions('genNet', ref.netOptions, 'code', item => `${item.name} (${item.code})`, 'X');
    setOptions('genPattern', ref.patterns, 'skuCode', item => `${item.name} (${item.skuCode})`, 'L0');
    updateGeneratedSku();
}

function updateGeneratedSku() {
    if (currentModalTab !== 'generate') return;

    const ref = window.SKU_REFERENCE;
    if (!ref) return;
    const type = document.getElementById('genType').value;
    const panels = parseInt(document.getElementById('genPanels').value) || 2;
    const width = parseInt(document.getElementById('genWidth').value) || 200;
    const height = parseInt(document.getElementById('genHeight').value) || 200;
    const frame = document.getElementById('genFrameColor').value;
    const glass = document.getElementById('genGlassColor').value;
    const net = document.getElementById('genNet').value;
    const pattern = document.getElementById('genPattern').value || 'L0';

    const sku = ref.buildProductCode({
        productType: type,
        panelCount: panels,
        height,
        width,
    ...
    generatedSkuCode = sku;

    const previewEl = document.getElementById('genSkuPreview');
    if (previewEl) {
        previewEl.textContent = sku;
    }

    document.getElementById('skuProductName').value = ref.buildProductName(sku);
}

async function fetchSkusForSelector(searchQueryStr) {
    if (!db) return;
    try {
        let query = db.from('sku_master').select('product_code, name, size');
...
async function handleAddSkuSubmit(e) {
    ...
    let productCode = '';
    if (currentModalTab === 'search') {
        productCode = document.getElementById('skuSelect').value;
    } else {
    ...
    const reasonText = document.getElementById('skuReason').value.trim();

    if (!productCode) {
        showToast('⚠️ กรุณาเลือกหรือสร้าง SKU', 'error');
        return;
    }
    ...
    try {
        // If tab is 'generate' and the product code doesn't exist in sku_master, insert into sku_master first.
        if (currentModalTab === 'generate') {
            const { data: skuExists, error: skuCheckError } = await db
                .from('sku_master')
                .select('product_code')
                .eq('product_code', productCode)
                .maybeSingle();
            ...
            if (!skuExists) {
                const parsed = window.SKU_REFERENCE?.parseProductCode(productCode);
                if (!parsed) throw new Error('รหัส SKU ที่สร้างไม่ตรงตามตารางมาตรฐาน');

                const lookupResults = await Promise.all([
                    db.from('products').upsert([{
                        product_prefix: parsed.productPrefix,
                        product_name: `${parsed.productTypeName} ${parsed.panelCount} บาน`
                    }], { onConflict: 'product_prefix' }),
                    db.from('aluminum_colors').upsert([{
                        color_code: parsed.frameColor,
                        color_name: parsed.frameColorName
                    }], { onConflict: 'color_code' }),
                    db.from('glass_colors').upsert([{
                        color_code: parsed.glassColor,
                        color_name: parsed.glassColorName
                    }], { onConflict: 'color_code' }),
                    db.from('patterns').upsert([{
                        pattern_code: parsed.patternCode,
                        pattern_name: parsed.patternName
                    }], { onConflict: 'pattern_code' })
                ]);
                const lookupFailure = lookupResults.find(result => result.error);
                if (lookupFailure?.error) throw lookupFailure.error;

                const { data: netRow, error: netError } = await db
                    .from('mosquito_nets')
                    .select('net_id')
                    .ilike('net_status', parsed.net.toLowerCase())
                    .maybeSingle();
                if (netError) throw netError;
                if (!netRow) throw new Error(`ไม่พบข้อมูลมุ้งรหัส ${parsed.net} ใน mosquito_nets`);

                const { error: insertSkuError } = await db.from('sku_master').insert({
                    product_code: productCode,
                    name: productName,
                    product_prefix: parsed.productPrefix,
                    size: parsed.size,
                    slots: parsed.panelCount,
                    pattern: parsed.patternName,
                    pattern_code: parsed.patternCode,
                    net_id: netRow.net_id,
                    frame_color_code: parsed.frameColor,
                    glass_color_code: parsed.glassColor
                });
                if (insertSkuError) throw insertSkuError;
            }
        }
        ...
        // Log movement
        const reasonFull = reasonText || `รับสินค้าสำเร็จรูปเข้าสต็อกสำรอง (สร้างใหม่/ดึงจาก SKU)`;
        const { error: logErr } = await db
            .from('production_backup_stock_log')
            .insert({
                product_code: productCode,
