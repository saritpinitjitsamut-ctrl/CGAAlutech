...
function updateGeneratedSku() {
    ...
    // ww & hh calculation (pad to 2 digits)
    const ww = Math.floor(width / 10).toString().padStart(2, '0');
    const hh = Math.floor(height / 10).toString().padStart(2, '0');

    // Generate product code
    const sku = `${type}${panels}P${ww}${hh}${frame}${glass}${net}${pattern}`;
    ...
    // Build Thai product name
    const typeLabel = {
        'SLD': 'ประตูบานเลื่อน',
        'SLWD': 'หน้าต่างบานเลื่อน',
        'SL': 'บานเลื่อน',
        'D': 'ประตู',
        'WD': 'หน้าต่าง'
    }[type] || type;
    ...
    const netLabel = net === 'N' ? 'มีมุ้ง' : 'ไม่มีมุ้ง';
    ...
    const generatedName = `${typeLabel} ${panels} บาน ${frameLabel}/${glassLabel} ${netLabel} ${patternLabel} (${sizeStr})`;
    
    document.getElementById('skuProductName').value = generatedName;
}
    ...
    try {
        ...
        if (currentModalTab === 'generate') {
            ...
            if (!skuExists) {
                const width = parseInt(document.getElementById('genWidth').value) || 200;
                const height = parseInt(document.getElementById('genHeight').value) || 200;
                const sizeFormat = `${width}x${height}`;

                const { error: insertSkuError } = await db
                    .from('sku_master')
                    .insert({
